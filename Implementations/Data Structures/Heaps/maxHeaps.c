...
void heapify(int *array, int i, int size) {
    ...
    while (i <= (n-1)/2) {
        ...
        if (2*i + 2 <= n && array[max] < array[2*i + 2])
            max = 2*i + 2;
...
int currMax(int *heap, int length) {
    return ((length != 0) ? heap[0] : -1);
}

///////////////////////////////////////////////////////////////////////////////

int extractMax(int *heap, int *length) {
...
void increaseKey(int *heap, int length, int k, int delta) {
    // searches for value k in the heap and increases it by delta
...
int main() {
    ...
    int extracted = extractMax(heap, &length); 
    print(heap, length);
    printf("extracted = %d \n\n", extracted);

    increaseKey(heap, length, 4, 3);
    increaseKey(heap, length, 1, 9);
    printf("heap after increasing = "); print(heap, length);
